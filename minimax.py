...
class Environment:
    ...
    def compute_minimax(self, node, depth, max_player = True):
...
class MiniMaxAgent:
    ...
    def act(self, node, environment):
        ...
        if goal_status == "Goal Reached":
            return f"Minimax value for root: {node.minimax_value}"
        else:
            value = environment.compute_minimax(node, self.depth)
            node.minimax_value = value   
            return value
